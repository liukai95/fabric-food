...
warehouseModel.findAllwarehouse = function(callback) {
    ...
    var args= ["WAREHOUSE000","WAREHOUSE999"];//参数

    fabricQueryModel.findData(options, fcn, args, callback);
}
//改进查询所有person数据
warehouseModel.findAllWarehouse2 = function(peerorg, callback) {
...
//插入仓库信息
warehouseModel.insertWarehouse = function(warehouseid, place, capacity, standard, personid, callback) {
    var invokeoptions = {
        user_id: config.USER_ID_ADMIN_ORG2,
        msp_id: config.MSP_ID_ORG2,
        channel_id: config.CHANNEL_ID,
    ...
    var fcn = "initWarehouse"; //invoke操作方法
    var args = [warehouseid, place, capacity, standard, personid]; //参数
