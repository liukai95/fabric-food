...
seedlingModel.findAllSeedling = function(callback) {
    ...
    var args= ["SEEDLING000","SEEDLING999"];//参数

    fabricQueryModel.findData(options, fcn, args, callback);
}
//改进查询所有seedling数据
seedlingModel.findAllSeedling2 = function(peerorg, callback) {
...
//插入秧苗信息
seedlingModel.insertSeedling = function(seedlingid, nurseryplace, startdate, personid, callback) {
    //链码和证书等参数
    var invokeoptions = {
        user_id: config.USER_ID_ADMIN_ORG1,
        msp_id: config.MSP_ID_ORG1,
    ...
    var fcn = "initSeedling"; //invoke操作方法
    var args = [seedlingid, nurseryplace, startdate, personid]; //参数
