...
input2warehouseModel.findAllInput2warehouse = function(callback) {
    ...
    var args= ["INPUTWAREHOUSE000","INPUTWAREHOUSE999"];//参数

    fabricQueryModel.findData(options, fcn, args, callback);
}
//改进查询所有input2warehouse数据
input2warehouseModel.findAllInput2warehouse2 = function(peerorg, callback) {
...
//插入信息
input2warehouseModel.insertInput2warehouse = function(inputid, warehouseid, personid, callback) {
    var invokeoptions = {
        user_id: config.USER_ID_ADMIN_ORG2,
        msp_id: config.MSP_ID_ORG2,
        channel_id: config.CHANNEL_ID,
    ...
    var fcn = "initInput2warehouse"; //invoke操作方法
    var args = [inputid, warehouseid, personid]; //参数
